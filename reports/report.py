...
class NoConstraints(Constraints):
    ...
    def __call__(self, _: str, __: int) -> bool:
        """No constraints on the answer."""
        return True
...
def html() -> None:
    ...
    with Path("report.html").open("w") as newfile:
        newfile.write(html)
...
def check() -> None:
    ...
    counter = 0
    for index, (answer, (_, constraints)) in enumerate(zip(answers, question_constraints.items(), strict=False), 1):
