...
# --------------------------------------------------
BACKEND_URL = "https://sign-ml-apicloud-434286151372.europe-west4.run.app"
...
def main():
    ...
    with st.spinner("Checking backend status..."):
        try:
            health_response = requests.get(HEALTH_URL, timeout=50)
            ...
            if st.button("🚀 Classify Traffic Sign", use_container_width=True, type="primary"):
                with st.spinner("🔄 Analyzing image..."):
                    try:
                        ...
                        # Prepare the file for upload - EXACTLY like reference code
                        files = {
                            "image": (
                                uploaded_file.name,
    ...
    with st.sidebar:
        ...
        try:
            ...
            if status == "ok" and is_loaded:
                ...
                st.caption(f"**Weights:** {weights_file.split('/')[-1] if '/' in str(weights_file) else weights_file}")
            else:
